...
既然这是一本关于低级 CPU 性能的书籍，我们将跳过讨论类似于刚才提到的更高层次的技术。相反，我们将讨论如何在关键路径上避免页面错误(page faults)、缓存未命中(cache misses)、TLB驱逐(TLB shootdowns)和核心节流(core throttling)。

### 避免小页面错误 {#sec:AvoidPageFaults}

虽然这个术语中包含了"小(minor)"这个词，但小页面错误对运行时延迟的影响绝对不可小觑。回想一下，当用户代码分配内存时，操作系统只承诺提供一页，但并不会立即通过给我们一个零初始化的物理页面来履行承诺。相反，它会等到用户代码第一次访问它时，然后操作系统才履行其职责。对新分配页面的第一次访问会触发一个小页面错误，这是一个由操作系统处理的硬件中断。小错误对延迟的影响可以从不到一微秒到几微秒不等，特别是如果你使用的是具有5级页面表而不是4级页面表的Linux内核。

你如何在你的应用程序中检测运行时小页面错误？一个简单的方法是使用`top`工具（添加`-H`选项以查看线程级别的视图）。添加`vMn`字段到默认的显示列选择中，以查看每个显示刷新间隔期间发生的小页面错误数量。[@lst:DumpTopWithMinorFaults]显示了在编译大型C++项目时，前10个进程的`top`命令的输出。额外的`vMn`列显示了在过去3秒内发生的小页面错误数量。
...
在[@lst:LockPagesAndNoRelease]中的代码调整了glibc malloc的三个设置：`M_MMAP_MAX`、`M_TRIM_THRESHOLD`和`M_ARENA_MAX`。

- 将`M_MMAP_MAX`设置为`0`会禁用对于大内存分配的底层`mmap`系统调用使用——这是必要的，因为当库尝试释放`mmap`分配的段回操作系统时，`mlockall`的效果可能会被`munmap`撤销，这会破坏我们这么做的目的。
- 将`M_TRIM_THRESHOLD`设置为`-1`可以防止glibc在`free`调用后将内存返回给操作系统。如前所述，这个选项对`mmap`分配的段没有影响。
- 最后，将`M_ARENA_MAX`设置为`1`可以防止glibc通过`mmap`为多个核心分配多个内存区域。请注意，后者会妨碍glibc分配器的多线程可扩展性特性。

综合这些设置，会强制glibc进行堆分配，这些分配在应用程序结束前不会将内存释放回操作系统。因此，在上述代码中`free(mem)`的最终调用后，堆的大小将保持不变。如果初始化时分配的空间足够大，任何后续的运行时调用`malloc`或`new`都将简单地重用这个预先分配/预先错误的堆区域中的空间。

更重要的是，由于之前的`mlockall`调用，`for`循环中预先错误的所有堆内存将保持在RAM中——选项`MCL_CURRENT`锁定当前映射的所有页面，而`MCL_FUTURE`锁定将来会成为映射的所有页面。使用`mlockall`的这种方式还有一个额外的好处，即由此进程生成的任何线程的栈也将被预先错误和锁定。对于更精细的页面锁定控制，开发者应该使用`mlock`系统调用，它允许你选择哪些页面应该保持在RAM中。这种技术的副作用是它会减少系统上其他正在运行的进程可用的内存量。

Windows应用程序的开发者应该查看以下API：使用`VirtualLock`锁定页面，使用带有`MEM_DECOMMIT`但不带有`MEM_RELEASE`标志的`VirtualFree`避免立即释放内存。

这只是防止运行时小页面错误的两种示例方法。这些技术中的一些或全部可能已经集成到如jemalloc、tcmalloc或mimalloc等内存分配库中。检查你选择的库的文档，看看有哪些可用的选项。
...
当一段代码路径一段时间没有运行时，它的指令和相关数据很可能从 I-cache 和 D-cache 中被驱逐出去。然后，就在我们需要运行这段关键的很少执行的代码时，我们遇到了 I-cache 和 D-cache 未命中惩罚，这可能会让我们输掉比赛。这就是*缓存预热(cache warming)*技术可以发挥作用的地方。
