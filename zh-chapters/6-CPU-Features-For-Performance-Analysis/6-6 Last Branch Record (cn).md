...
非常重要的一点是，只有被执行的分支（taken branches）才会被记录。[@lst:LogBranches]展示了如何跟踪分支结果的一个例子。这段代码表示一个循环，其中包含三个可能改变程序执行路径的指令，即循环回边(loop backedge)`JNE`（1）、条件分支`JNS`（2）、函数调用`CALL`（3），以及从这个函数返回（4，未显示）。
...
LBR寄存器的作用类似于一个环形缓冲区，它不断地被覆盖，并提供最近32个分支结果。每个LBR条目由三个64位值组成：

- 分支的源地址（`From IP`）。
- 分支的目标地址（`To IP`）。
- 操作的元数据，包括误预测和经过的周期时间信息。

除了源地址和目标地址之外，保存的额外信息也有重要的应用，我们将在后面讨论。

当采样计数器溢出并触发性能监控中断（PMI）时，LBR记录会冻结，直到软件捕获LBR记录并恢复收集。

LBR收集可以限制在一组特定的分支类型上，例如，用户可以选择只记录函数调用和返回。在对[@lst:LogBranches]中的代码应用这样的过滤器时，我们只会在历史记录中看到分支（3）和（4）。用户还可以过滤进出条件和无条件跳转、间接跳转和调用、系统调用、中断等。在Linux perf中有一个`-j`选项，用于启用/禁用记录各种分支类型。

默认情况下，LBR数组作为一个环形缓冲区，捕获控制流转换。然而，LBR数组的深度是有限的，这可能是在分析某些应用程序时的一个限制因素，在这些应用程序中，执行流的转换伴随着大量的叶子函数调用。这些对叶子函数的调用及其返回，很可能会将主执行上下文从LBR中移出。再次考虑[@lst:LogBranches]中的例子。假设我们想从LBR历史记录中展开调用栈，并且我们配置LBR只捕获函数调用和返回。如果循环运行数千次，考虑到LBR数组只有32个条目深，我们很可能只会看到16对条目（3）和（4）。在这种情况下，LBR数组充满了叶子函数调用，这些调用无助于我们展开当前的调用栈。

这就是为什么LBR支持调用栈模式。启用此模式后，LBR数组像以前一样捕获函数调用，但当执行返回指令时，最后捕获的分支（`call`）记录会以后进先出（LIFO）的方式从数组中清除。因此，与已完成的叶子函数相关的分支信息不会被保留，同时保留了主线执行路径的调用栈信息。当以这种方式配置时，LBR数组模拟了一个调用栈，其中`CALL`被“推入”（pushed），`RET`被“弹出”（popped）条目。如果你的应用程序中的调用栈深度从未超过32个嵌套帧，LBRs将为你提供非常准确的信息。[@IntelOptimizationManual, Volume 3B, Chapter 19 Last Branch Records]
...
AMD处理器也在AMD Zen4处理器上支持最后分支记录（LBR）。Zen4有16对“from”和“to”地址记录以及一些额外的元数据。与Intel LBR类似，AMD处理器有能力记录各种类型的分支。与Intel LBR的主要区别在于，AMD处理器目前还不支持调用栈模式，因此LBR特性不能用于调用栈收集。另一个值得注意的区别是AMD LBR记录中没有周期计数字段。更多详细信息请参见 [@AMDProgrammingManual, 13.1.1.9 Last Branch Stack Registers]。

从Linux内核6.1开始，除非另有明确说明，Linux 'perf'在AMD Zen4处理器上支持我们下面讨论的分支分析用例。收集AMD LBRs的Linux `perf`命令使用相同的`-b`和`-j`选项。
...
这些信息可以用于对该基本块进行细粒度调整。此示例可能受益于内存预取，我们将在 [@sec:memPrefetch] 中讨论。此外，周期计数信息可用于循环迭代的计时，其中每个循环迭代都以一个已采取的分支（后向边缘(back edge)）结束。
...
我们将在后面的 [@sec:secPGO] 部分讨论基于优化分析文件的优化 (PGO)，这里先简要提一下。 分支记录机制可以为优化编译器提供优化分析文件反馈数据。 想象一下，我们可以将我们在前面部分发现的所有数据反馈给编译器。 在某些情况下，这些数据无法使用传统的静态代码检测工具获得，因此分支记录机制不仅因开销更低而成为更好的选择，而且还能提供更丰富的优化分析文件数据。 依赖硬件 PMU 收集数据进行的 PGO 工作流程越来越流行，一旦 AMD 和 ARM 的支持成熟，可能会迅速发展。
